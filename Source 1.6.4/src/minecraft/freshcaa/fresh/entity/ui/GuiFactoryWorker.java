package freshcaa.fresh.entity.ui;
...
public class GuiFactoryWorker extends GuiContainer
{
    ...
    public GuiFactoryWorker(InventoryPlayer par1InventoryPlayer, FactoryWorker factoryWorker, World par3World, String par4Str)
    {
        super(new ContainerMerchant(par1InventoryPlayer, factoryWorker, par3World));
    ...
    {
        super.updateScreen();
        EntityAITasks aiTasks = this.factoryWorker.tasks;

        if (aiTasks != null)
        {
            this.nextRecipeButtonIndex.enabled = this.currentTaskIndex < aiTasks.taskEntries.size() - 1;
            this.previousRecipeButtonIndex.enabled = this.currentTaskIndex > 0;
        }
    }
    ...
    protected void actionPerformed(GuiButton par1GuiButton)
    {
        ...
        if (flag)
        {
        	this.factoryWorker.clearTasks();
        	this.factoryWorker.setActiveTask(this.currentTaskIndex);
            ((ContainerMerchant)this.inventorySlots).setCurrentRecipeIndex(this.currentTaskIndex);
            ByteArrayOutputStream bytearrayoutputstream = new ByteArrayOutputStream();
            DataOutputStream dataoutputstream = new DataOutputStream(bytearrayoutputstream);

            try
            {
                dataoutputstream.writeInt(this.currentTaskIndex);
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3)
    {
        ...
        MerchantRecipeList merchantrecipelist = this.factoryWorker.getRecipes(this.mc.thePlayer);

        if (merchantrecipelist != null && !merchantrecipelist.isEmpty())
        {
            int i1 = this.currentTaskIndex;
