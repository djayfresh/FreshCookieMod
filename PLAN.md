...
- Mod logo done in 2.1.1 (`src/main/resources/logo.png`, generated by `tools/release_art.py`). The GUI sheet and the Factory Worker spawn egg were never touched.
...
### Housekeeping
- Remote branches `neoforge-26.3`, `table-model` and `archive/port-1.8` are deleted; only `master` and `dev` remain on GitHub.
