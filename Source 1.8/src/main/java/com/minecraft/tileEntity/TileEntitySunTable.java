...
public class TileEntitySunTable extends TileEntity implements ISidedInventory
{
	...
	public void update()
	{
		...
		if(!worldObj.isRemote)
		{
			...
			{
				flag1 = true;
				SunTable.setState(isBurning(), worldObj, pos); // this should update the actually furnace to
	...
	{
		return worldObj.canBlockSeeSky(pos.add(0, 2, 0))
				&& worldObj.getBlockLightOpacity(pos.add(0, 1, 0)) >= 14;
	}
	...
	public boolean isUseableByPlayer(EntityPlayer entityplayer)
	{
		return worldObj.getTileEntity(pos) != this ? false
				: entityplayer.getDistanceSq(new BlockPos(
	...
	public int[] getSlotsForFace(EnumFacing side)
	{
		return side == EnumFacing.DOWN ? slots_bottom
				: (side == EnumFacing.UP ? slots_sides : slots_top);
	}
	...
	{
		return face.getIndex() != 1 || i != 1
				|| itemstack.getItem() == Items.bucket; // not from the top, not the fuel, and not a bucket
	}
