...
import com.minecraft.world.MacadamiaTreeWorldGenerator;
import com.minecraft.world.PecanTreeWorldGenerator;
...
public class WorldGeneratorDjf implements IWorldGenerator
{
			...
			IChunkProvider chunkGenerator, IChunkProvider chunkProvider)
	{
		...
		{
		case -1: generateSurface(world, random,chunkX*4,chunkZ*4);
		case 0:
			...
			int BlockZ)
	{
		...
		for(int i = 0; i < 10; i++) //the size represends the proability of spwaning 
		{
			...
			(new PecanTreeWorldGenerator(false, 6, 0, 0, false)).generate(world, random, new BlockPos(xCoord, yCoord, zCoord));
		}
		...
		for(int i = 0; i < 10; i++) //the size represends the proability of spwaning 
		{
			...
			(new MacadamiaTreeWorldGenerator(false, 3, 0, 0, false)).generate(world, random, new BlockPos(xCoord2, yCoord2, zCoord2));
		}
	}
}
